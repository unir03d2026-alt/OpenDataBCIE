...
def run_etl(config_path, run_id=None):
    ...
    # Column Renaming
    rename_list = config['data']['column_renames']
    # Convert list of dicts to single dict if necessary, or just use as is if it's a dict
    # YAML loading might interpret "key: value" inside list slightly differently depending on structure
    # Checking if it's a dict or list of dicts. Standard config usually has a direct dict.
    rename_map = rename_list 
    ...
    if 'Anio_Origen' in df.columns:
        ...
        # Fallback if Anio_Origen was renamed to Anio or something specific, check config
        # Assuming Anio_Origen exists in raw or was renamed.
        # If renamed to 'Anio', use that.
        if 'Anio' in df.columns:
             df['Fecha_Aprobacion'] = pd.to_datetime(df['Anio'].astype(str) + '-01-01')
        else:
             # Try finding the 'Anio_Origen' equivalent in values of rename_map
             pass
